...
class GSKK:
	...
	def savepoints(self):
		...
		#print zerolist,self.num.get()
		#f = datfit.fit_data.fit(len(points),self.num.get(),freq,zerolist)
		...
		if self.columns != 3:
			...
			for i in range(len(self.other_column)):
				fittedanchors[i][:] = [zerolist[i],self.other_column[i], \
						self.fomega_entry[i].get()]
		#print fittedanchors
	...
	def findanchors(self):
		...
		#f = cheby.chebyanchors.cheby(len(points),self.ancnum.get(), \
		#				self.wmin.get(),self.wmax.get())
